...
/*txPWR Value*/
static u8t ber_oid_txpwr_int[] PROGMEM     				= {0x2b, 0x06, 0x01, 0x04, 0x01, 0x81, 0xac, 0x5d, 0x64, 0x84, 0x58, 0x05, 0x00};
static ptr_t oid_txpwr_int PROGMEM         				= {ber_oid_txpwr_int, 13};
...
/* set and get txOutputPower */

/** \brief LED DS1 for @ANY module get function*/
s8t getTxOutputPower(mib_object_t* object, u8t* oid, u8t len)
{
    object->varbind.value.i_value = rf230_get_txpower();
    return 0;
}


/** \brief set LED DS1 for @ANY module*/
s8t setTxOutputPower(mib_object_t* object, u8t* oid, u8t len, varbind_value_t value)
{
	printf("set TxPower to:%d\n",value.i_value);
    if (value.i_value >=0 || value.i_value <=15) 
	{
		rf230_set_txpower(value.i_value); //0=3dbm 15=-17.2dbm
		return 0;
	}
	else
...
s8t mib_init()
{
    ...
    if (add_scalar(&oid_txpwr_int, 0, BER_TYPE_INTEGER, 0, &getTxOutputPower, &setTxOutputPower) == -1)
	{
        return -1;
    }
